...
the first parameter from the reducer function will be the current state.

the second param from the reducer function can be anything, including a simple
param, an obj with a set of params, or either a function that computes and
returns the new state.

as the reducer gets more complex, we can follow the conventions and use a
action/type strategy, which consists in sending the second param as an object
with an `type` prop that will define what behavior/action/new states will be
computed

`useReducer` also receiver a third param which is a function to do lazy
initialization
