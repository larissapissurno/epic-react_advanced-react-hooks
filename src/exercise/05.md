...
`useImperativeHandle` is a hook that customizes the instance value that is
exposed when using ref in a parent component. This is typically used to expose
imperative methods from a child component to the parent component.

Key Points:

- useImperativeHandle should be used with forwardRef to allow refs to be passed
  down to the child component.
- It customizes the instance value (methods or properties) exposed to the parent
  component.
- This hook is useful for exposing imperative actions to parent components that
  can't be easily handled by props and state alone.
